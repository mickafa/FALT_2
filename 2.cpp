...
double  r_max, r_min,       /// апогей и перигей парусника на витке
        ...
        phi_max;            /// угол phi парусника в момент прохождения апогея витка
...
/// координаты Земли и Луны в момент прохождения парусником апогея
    static char format[] =
    "%5d; %7.3lf; %7.3lf; %8.2lf; %8.2lf; %7.1lf\n";
...
int ticker = TICKER ;                       /// периодичность записи репортов, в витках
   ...
   assert ( fo ) ;
    ...
    fprintf ( fp, "round; Apogee;   Perigee; v_max;    v_min;     phi_max\n");
     printf (     "round; Apogee;   Perigee; v_max;    v_min;     phi_max\n");
...
///  Последние 800 (раньше было 1000) витков дрейфуем в любом случае, для тестирования стабильности получившейся орбиты
...
///  Закончилось неудачно, пришлось на 1535 витке нажимать на красную кнопку, записывать последний виток и завершать
                ...
                else {     /// последние 800 витков - дрейф для тестирования стабильности орбиты
